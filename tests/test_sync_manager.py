...
class TestHAToTopologySync:
    ...
    async def test_reconcile_renames_managed_shadow_to_topomation_suffix(
        self,
        hass: HomeAssistant,
        sync_manager: SyncManager,
        loc_mgr: LocationManager,
        ...
        """Managed-shadow names should reconcile to Topomation suffixing."""
        area_reg = ar.async_get(hass)
        floor_reg = fr.async_get(hass)
        main_floor = floor_reg.async_create("Main Floor")
        area_reg.async_create("Main Floor", floor_id=main_floor.floor_id)
        shadow_area = area_reg.async_create("Main Floor Internal", floor_id=main_floor.floor_id)

        await sync_manager.import_all_areas_and_floors()

        floor_loc_id = f"floor_{main_floor.floor_id}"
        shadow_loc_id = f"area_{shadow_area.id}"

        floor_loc = loc_mgr.get_location(floor_loc_id)
        shadow_loc = loc_mgr.get_location(shadow_loc_id)
        assert floor_loc is not None
        assert shadow_loc is not None

        shadow_meta = dict(shadow_loc.modules.get("_meta", {}))
        shadow_meta.update(
            {
                "type": "area",
                "role": "managed_shadow",
        ...
        loc_mgr.set_module_config(shadow_loc_id, "_meta", shadow_meta)

        floor_meta = dict(floor_loc.modules.get("_meta", {}))
        floor_meta["shadow_area_id"] = shadow_loc_id
        loc_mgr.set_module_config(floor_loc_id, "_meta", floor_meta)

        sync_manager.reconcile_managed_shadow_areas()

        updated_shadow_area = area_reg.async_get_area(shadow_area.id)
        updated_shadow_loc = loc_mgr.get_location(shadow_loc_id)
        ...
        assert "Internal" not in updated_shadow_area.name
