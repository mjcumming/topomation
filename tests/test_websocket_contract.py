...
@pytest.mark.asyncio
async def test_locations_reorder_uses_compat_path_for_legacy_parent_reparent_error(
    hass: HomeAssistant,
) -> None:
    """Legacy manager error still allows valid subtree reparent via compatibility path."""
    store = Store(hass, STORAGE_VERSION, STORAGE_KEY_CONFIG)
    await store.async_save({"locations": []})
    ...
    original_reorder = loc_mgr.reorder_location

    def reorder_with_legacy_parent_block(
        location_id: str,
        ...
        if (
            ...
            and location.parent_id != new_parent_id
            and any(child.parent_id == location_id for child in loc_mgr.all_locations())
        ):
            raise ValueError(
                "Parent locations cannot move under a different parent. "
                "Reorder it within the current level."
            )
        return original_reorder(location_id, new_parent_id, new_index)

    connection = _fake_connection()
    with patch.object(loc_mgr, "reorder_location", side_effect=reorder_with_legacy_parent_block):
        handle_locations_reorder(
            hass,
            connection,
            {
