...
export class HomeTopologyPanel extends LitElement {
    ...
    return html`
      <div class="panel-container">
        ...
        <div class="panel-right">
          <div class="header">
            <div class="header-title">${selectedLocation ? selectedLocation.name : 'Location Details'}</div>
            <div class="header-subtitle">${selectedLocation ? selectedLocation.id : 'Select a location to configure modules'}</div>
          </div>
  ...
  private _renderDialogs() {
    ...
    return html`
      ...
      <ht-rule-dialog
        ...
        @dialog-closed=${() => { this._ruleDialogOpen = false; this._editingRule = undefined; }}
  ...
  private async _loadLocations(silent = false): Promise<void> {
    ...
    try {
      ...
      console.log("Calling WebSocket API: home_topology/locations/list");
      ...
      this._locations = [...uniqueLocations];
  ...
  private async _handleRuleSaved(e: CustomEvent): Promise<void> {
    ...
    this._showToast(`Rule "${rule.name}" saved`, 'success');
    await this._loadLocations(true);
  ...
  private async _subscribeToUpdates(): Promise<void> {
    ...
    try {
      this._unsubUpdates = await this.hass.connection.subscribeEvents(
        (event: any) => {
          this._logEvent("ha", "home_topology_updated", event?.data || {});
          this._scheduleReload(true);
        },
        "home_topology_updated"
      );
    } catch (err) {
      console.warn("Failed to subscribe to home_topology_updated events", err);
      this._logEvent("error", "subscribe failed: home_topology_updated", String(err));
    }
    ...
    try {
      this._unsubOccupancyChanged = await this.hass.connection.subscribeEvents(
        (event: any) => {
          ...
          this._logEvent("ha", "home_topology_occupancy_changed", { locationId, occupied });
        },
        "home_topology_occupancy_changed"
      );
    } catch (err) {
      console.warn("Failed to subscribe to home_topology_occupancy_changed events", err);
      this._logEvent("error", "subscribe failed: home_topology_occupancy_changed", String(err));
    }
  }
