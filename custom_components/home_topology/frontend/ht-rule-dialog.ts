...
import type { HomeAssistant, Location, AutomationRule } from "./types";
...
export class HtRuleDialog extends LitElement {
  ...
  @property({ attribute: false }) public rule?: AutomationRule;

  @state() private _config: Partial<AutomationRule> = {};
  ...
  private async _handleSubmit() {
    ...
    try {
      const currentConfig = this.location.modules?.automation || { rules: [] };
      ...
      if (this.rule) {
        ...
        const index = rules.findIndex((r) => r.id === this.rule!.id);
        if (index !== -1) {
